...
class StorageMaintenanceTests(unittest.TestCase):
    def test_clear_queue_removes_only_pending_work(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            has_reply_target = storage.has_bot_reply_target(topic_id=10, reply_to_post_number=2)

        self.assertEqual(result["pending_replies_deleted"], 1)
        self.assertEqual(result["manual_commands_deleted"], 1)
        self.assertEqual(result["pending_reply_targets_deleted"], 1)
        self.assertEqual(len(remaining), 1)
        self.assertEqual(remaining[0].status, "completed")
        self.assertFalse(has_reply_target)
    ...
    def test_reset_database_removes_all_local_state(self) -> None:
        ...
        self.assertEqual(result["pending_replies_deleted"], 1)
        self.assertEqual(result["manual_commands_deleted"], 1)
        self.assertEqual(result["reply_targets_deleted"], 1)
    ...
    def test_stats_summary_reports_queue_and_handled_counts(self) -> None:
        ...
        self.assertEqual(summary["reply_targets"], 1)

    def test_reply_targets_track_completed_notification_and_manual_replies(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            storage = BotStorage(Path(temp_dir) / "bot.sqlite3")
            storage.enqueue_job(
                notification_id=11,
            ...
            storage.record_handled(
                ...
                reason="done",
                response_post_id=99,
                handled_at="2026-01-01T00:01:00+00:00",
            )
            command_id = storage.enqueue_manual_command(
                post_url="https://forum.example.com/t/topic/101/3",
                user_request="reply",
                created_at="2026-01-01T00:00:00+00:00",
            )
            storage.schedule_manual_command(
            ...
            storage.complete_manual_command(
                command_id,
                response_post_id=100,
                completed_at="2026-01-01T00:02:00+00:00",
            )

            summary = storage.stats_summary()
            has_notification_target = storage.has_bot_reply_target(topic_id=100, reply_to_post_number=2)
            has_manual_target = storage.has_bot_reply_target(topic_id=101, reply_to_post_number=3)

        self.assertTrue(has_notification_target)
        self.assertTrue(has_manual_target)
        self.assertEqual(summary["reply_targets"], 2)
